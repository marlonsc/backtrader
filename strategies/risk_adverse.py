...
class AverageVolatility(bt.Indicator):
    ...
    def __init__(self):
        ...
        self.pct_change = 100.0 * (self.data.close(-1) - self.data.close(-2)) / self.data.close(-2)
...
class RiskAverseStrategy(bt.Strategy, TradeThrottling):
    ...
    def __init__(self):
        ...
        self.volatility = AverageVolatility(self.data, period=self.params.volatility_period)
        ...
        self.volume_ma = bt.indicators.SimpleMovingAverage(self.datavolume, period=self.params.vol_period)
        ...
        self.log(f'Strategy initialized with volatility threshold: {self.params.volatility_threshold}%')
        self.log(f'Using stop loss: {self.params.use_stop_loss}, Stop loss percentage: {self.params.stop_pct}%')
        self.log(f'Using trailing stop: {self.params.use_trailing_stop}, Trailing stop percentage: {self.params.trail_pct}%')
        self.log(f'Trade throttling: {self.params.trade_throttle_days} days between trades')
    ...
    def calculate_position_size(self, price):
        ...
        if self.params.use_stop_loss:
            ...
            if risk_per_share > 0:
                ...
                equity_based_size = int((value * self.params.position_size) / current_price)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if len(self) % 10 == 0:
                self.log(f"DEBUG - Close: {self.dataclose[0]:.2f}, Volatility: {self.volatility.avg_volatility[0]:.2f}%, " +
                         f"New High: {'Yes' if cond_2 else 'No'}, " +
                         f"Volume: {self.datavolume[0]:.0f}, High-Low Diff: {self.high_low_diff.diff[0]:.3f}", 
                         level='debug')
            ...
            if cond_1 and cond_2 and cond_3 and cond_4:
                ...
                self.log(f'BUY SIGNAL: Volatility: {self.volatility.avg_volatility[0]:.2f}%, ' +
                        f'New High: Yes, Volume: {self.datavolume[0]:.0f}, High-Low Diff: {self.high_low_diff.diff[0]:.3f}')
            ...
            if self.params.use_trailing_stop and self.trail_price is not None:
                ...
                if self.datahigh[0] > self.highest_price:
                    ...
                    new_trail = self.highest_price * (1.0 - self.params.trail_pct / 100.0)
            ...
            if exit_count >= self.params.exit_count:
                self.log(f'SELL CREATE (Strategy Exit): {exit_count} exit conditions met')
                self.order = self.sell()
                return
    ...
    def stop(self):
        ...
        self.log(f'Strategy Settings: Volatility({self.params.volatility_period}, {self.params.volatility_threshold}%), ' + 
                f'High-Low({self.params.high_low_period}, {self.params.high_low_threshold}), ' +
...
def parse_args():
    ...
    # Volatility parameters
    parser.add_argument('--volatility-period', '-vp',
                        default=20, type=int,
                        help='Period for volatility calculation')
    
    parser.add_argument('--volatility-threshold', '-vt',
                        default=8.0, type=float,
                        help='Maximum allowed volatility percentage')
    
    # High-Low parameters
    parser.add_argument('--high-low-period', '-hlp',
                        default=60, type=int,
                        help='Period for high-low difference calculation')
    
    parser.add_argument('--high-low-threshold', '-hlt',
                        default=0.3, type=float,
                        help='Maximum allowed high-low difference ratio')
    
    # Volume parameters
    parser.add_argument('--vol-period', '-volp',
                        default=5, type=int,
                        help='Period for volume moving average')
    
    parser.add_argument('--vol-threshold', '-volt',
                        default=100000, type=int,
                        help='Minimum required volume')
    
    # Exit parameters
    parser.add_argument('--exit-count', '-ec',
                        default=2, type=int,
...
def main():
    ...
    print(f'- Volatility: Max {args.volatility_threshold}% over {args.volatility_period} periods')
    print(f'- High-Low Diff: Max {args.high_low_threshold} over {args.high_low_period} periods')
    ...
    print(f'- Position Sizing: Using {args.position_percent}% of equity per trade (max {args.max_position}%)')
