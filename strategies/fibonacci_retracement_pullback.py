...
class FibonacciPullbackStrategy(bt.Strategy, TradeThrottling):
    ...
    def is_pullback(self):
        ...
        for fib_line in [self.fib.fib382, self.fib.fib500, self.fib.fib618]:
            ...
            price_near_fib = abs(self.dataclose[0] - fib_level) / fib_level < (self.p.price_tolerance / 100)
            
            # RSI should be oversold
            rsi_oversold = self.rsi[0] < self.p.rsi_lower
            
            # Volume should be increasing
            vol_increasing = self.datavolume[0] > self.datavolume[-1] * self.p.volume_mult
    ...
    def calculate_position_size(self):
        ...
        max_size = int((self.broker.getcash() * self.p.max_position / 100) / current_price)
    ...
    def should_exit_trade(self):
        ...
        if self.p.use_stop and self.datalow[0] <= self.stop_loss:
            self.log(f'STOP LOSS TRIGGERED: Price: {self.datalow[0]:.2f}, Stop: {self.stop_loss:.2f}')
            return True
        
        # Check take profit
        if self.datahigh[0] >= self.take_profit:
            self.log(f'TARGET PROFIT REACHED: Price: {self.datahigh[0]:.2f}, Target: {self.take_profit:.2f}')
            return True
        
        # Check trailing stop
        if self.p.use_trail and self.datalow[0] <= self.trailing_stop:
            self.log(f'TRAILING STOP TRIGGERED: Price: {self.datalow[0]:.2f}, Stop: {self.trailing_stop:.2f}')
            return True
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.is_uptrend() and self.is_pullback():
                ...
                if size > 0:
                    self.log(f'BUY SIGNAL: Price: {self.dataclose[0]:.2f}, Pullback to Fibonacci level')
    ...
    def stop(self):
        ...
        self.log('NOTE: This strategy is specifically designed for PULLBACKS WITHIN UPTRENDS')
        self.log('      It performs poorly in bear markets or during major corrections')
        self.log('      Ideal conditions: Stocks showing strong momentum with healthy pullbacks')
...
def parse_args():
    ...
    # Trend parameters
    parser.add_argument('--trend-period', '-tper', type=int, default=20,
                       help='Period for trend calculation')
    parser.add_argument('--trend-strength', '-ts', type=float, default=5.0,
    ...
    parser.add_argument('--rsi-upper', '-ru', type=int, default=70,
                       help='RSI upper threshold')
    parser.add_argument('--rsi-lower', '-rl', type=int, default=30,
                       help='RSI lower threshold')
    
    # Fibonacci parameters
    parser.add_argument('--swing-lookback', '-swl', type=int, default=50,
                       help='Bars to look back for swing points')
    parser.add_argument('--bounce-threshold', '-bt', type=float, default=0.5,
                       help='Minimum bounce % from level')
    parser.add_argument('--volume-mult', '-vm', type=float, default=1.5,
                       help='Volume increase factor for confirmation')
    parser.add_argument('--price-tolerance', '-pt', type=float, default=0.5,
                       help='How close price needs to be to Fibonacci level (%)')
    
    # Exit parameters
    parser.add_argument('--use-stop', '-us', action='store_true', default=True,
                       help='Use stop loss')
    parser.add_argument('--stop-pct', '-sp', type=float, default=2.0,
                       help='Stop loss % below entry')
    parser.add_argument('--target-pct', '-tp', type=float, default=5.0,
                       help='Take profit % above entry')
    parser.add_argument('--use-trail', '-ut', action='store_true',
                       help='Use trailing stop')
    parser.add_argument('--trail-pct', '-trp', type=float, default=2.0,
                       help='Trailing stop %')
    
    # Position sizing
    parser.add_argument('--risk-percent', '-riskp', type=float, default=1.0,
...
def main():
    ...
    print(f'RSI Parameters: Period={args.rsi_period}, Upper={args.rsi_upper}, Lower={args.rsi_lower}')
    ...
    print('\n--- Starting Backtest ---\n')
    print('*** IMPORTANT: This strategy is specifically designed for PULLBACKS WITHIN UPTRENDS ***')
    print('It performs poorly in bear markets or during major corrections. Best used with stocks')
    print('showing strong momentum with healthy pullbacks to key Fibonacci retracement levels.\n')
