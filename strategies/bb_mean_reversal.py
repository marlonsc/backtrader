    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.dataclose[0] <= active_stop:
                self.log(
                    f"STOP TRIGGERED: Close {self.dataclose[0]:.2f}, Stop"
                    f" {active_stop:.2f}"
                )
            ...
            # Profit-taking exit
            if (
                self.datasrc[0] >= self.bbands.top[0]
                and self.rsi[0] > self.p.rsi_overbought
            ):
                self.log(
                    f"PROFIT EXIT: Close {self.datasrc[0]:.2f}, Upper BB"
                    f" {self.bbands.top[0]:.2f}, RSI {self.rsi[0]:.2f}"
                )
                self.order = self.close(exectype=bt.Order.Close)
                return
        ...
        # Entry logic
            ...
            self.datasrc[0] <= self.bbands.bot[0]
            and self.rsi[0] < self.p.rsi_oversold
            and self.volatility_filter()
        ):
        ...
        if order.status == order.Completed:
            ...
            if order.isbuy():
                ...
                self.log(
                    ...
                    f" {order.executed.size:.2f}, Stop {self.stop_price:.2f}, Comm"
                    f" {order.executed.comm:.2f}"
                )
            else:
                profit = (
                    (order.executed.price - self.entry_price) * order.executed.size
                    if self.entry_price
                    else 0
                )
                ...
                if self.entry_bar:
                    ...
                    duration = (
                        exit_bar - self.entry_bar
                    ).total_seconds() / 3600  # Convert to hours (1h timeframe)
                ...
                self.log(
                    ...
                    f" {order.executed.size:.2f}, Profit {profit:.2f}, Comm"
                    f" {order.executed.comm:.2f}"
                )
                self.entry_price = None
                self.stop_price = None
...
def parse_args():
    ...
    parser.add_argument(
        "--fromdate", "-f", default="2024-01-01", help="Start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--todate", "-t", default="2024-12-31", help="End date (YYYY-MM-DD)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--plot", "-pl", action="store_true", help="Plot trading activity"
    )
