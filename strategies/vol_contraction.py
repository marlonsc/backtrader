...
class VCPPattern(bt.Indicator):
    ...
    def __init__(self):
        ...
        self.short_high = bt.indicators.Highest(self.data.high, period=self.p.period_short)
        ...
        self.long_high = bt.indicators.Highest(self.data.high, period=self.p.period_long)
        self.long_low = bt.indicators.Lowest(self.data.low, period=self.p.period_long)
        self.long_range = self.long_high - self.long_low
        
        # Highest close
        self.highest_close = bt.indicators.Highest(self.data.close, period=self.p.highest_close)
        
        # Volume average
        self.avg_volume = bt.indicators.SimpleMovingAverage(self.data.volume, period=self.p.mean_vol)
    
    def next(self):
        # Volatility contraction
        vol_contraction = self.short_range[0] < (self.long_range[0] * self.p.period_long_discount)
        ...
        self.lines.vcp[0] = 1.0 if (vol_contraction and near_high and vol_less_than_avg) else 0.0
...
class VCPStrategy(bt.Strategy, TradeThrottling):
    ...
    ** IMPORTANT: This strategy is specifically designed for stocks forming bases
    ...
    def is_uptrend(self):
        ...
        return (self.dataclose[0] > self.sma_long[0] and 
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.vcp[0] == 1.0:
                ...
                if (self.is_uptrend() and 
                    self.is_narrow_channel() and 
                    self.is_sufficient_liquidity()):
            ...
            if self.p.trail_stop and self.trail_price is not None:
                ...
                if self.datahigh[0] > self.buyprice:
                    new_trail_price = self.datahigh[0] * (1.0 - self.p.trail_percent / 100.0)
    ...
    def stop(self):
        ...
        self.log('NOTE: This strategy is designed for stocks forming bases before breakouts', doprint=True)
        self.log('      It works best in bull markets with sector rotation', doprint=True)
...
def parse_args():
    ...
    # VCP parameters
    parser.add_argument('--period-short', type=int, default=10,
                       help='Short-term lookback period for volatility')
    
    parser.add_argument('--period-long', type=int, default=60,
                       help='Long-term lookback period for volatility')
    
    parser.add_argument('--period-long-discount', type=float, default=0.7,
                       help='Discount factor for long period')
    
    parser.add_argument('--highest-close', type=int, default=100,
                       help='Lookback period for highest close')
    
    parser.add_argument('--mean-vol', type=int, default=20,
                       help='Period for volume average calculation')
    
    # Moving average parameters
    parser.add_argument('--sma-long', '-sl', type=int, default=250,
                       help='Long-term SMA period for trend identification')
    
    parser.add_argument('--sma-short', '-ss', type=int, default=60,
                       help='Short-term SMA period for exit signals')
    
    # Price channel parameters
    parser.add_argument('--recent-price-period', '-rpp', type=int, default=20,
                       help='Period for narrow channel calculation')
    
    parser.add_argument('--narrow-factor', '-nf', type=float, default=0.7,
                       help='Factor for determining narrow channel')
    
    # Liquidity filter
    parser.add_argument('--min-dollar-volume', '-mdv', type=float, default=2000000,
                       help='Minimum dollar volume for liquidity filter')
    
    # Position sizing
    parser.add_argument('--position-percent', '-pp', type=float, default=20.0,
                       help='Position size as percentage of equity')
    
    parser.add_argument('--max-position', '-mp', type=float, default=95.0,
                       help='Maximum position size as percentage')
    
    # Risk management
    parser.add_argument('--stop-loss', '-stl', type=float, default=7.0,
                       help='Stop loss percentage')
    
    parser.add_argument('--trailing-stop', '-ts', action='store_true',
                       help='Enable trailing stop loss')
    
    parser.add_argument('--trail-percent', '-tp', type=float, default=10.0,
