...
class RSIOverboughtOversoldStrategy(bt.Strategy, TradeThrottling):
    ...
    def should_buy(self):
        ...
        if rsi_below_oversold and rsi_turning_up and oversold_bars >= self.p.confirmation:
            ...
            if self.p.use_stoch:
                ...
                stoch_turning_up = (self.stoch.percK[0] > self.stoch.percK[-1] and 
                                   self.stoch.percD[0] > self.stoch.percD[-1])
    ...
    def should_sell(self):
        ...
        # Check stop loss
        if self.p.use_stop and self.stop_price is not None and self.datalow[0] <= self.stop_price:
        ...
        if self.take_profit_price is not None and self.datahigh[0] >= self.take_profit_price:
            self.log(f'TAKE PROFIT REACHED: Price: {self.datahigh[0]:.2f}, Target: {self.take_profit_price:.2f}')
            return True
        
        # Update trailing stop if enabled
        if self.p.use_trail and self.trail_price is not None and self.datahigh[0] > self.highest_price:
        ...
        # Check trailing stop
        if self.p.use_trail and self.trail_price is not None and self.datalow[0] <= self.trail_price:
        ...
        if self.rsi[0] >= self.p.overbought:
            if self.rsi[0] <= self.rsi[-1]:  # RSI starting to turn down
                ...
                if self.sell_signal_count >= self.p.confirmation:
                    self.log(f'RSI OVERBOUGHT SIGNAL: RSI: {self.rsi[0]:.2f}, Price: {self.dataclose[0]:.2f}')
                    return True
        else:
        ...
        if self.p.use_stoch and self.is_stoch_valid():
            if self.stoch.lines.percK[0] >= 80:  # Stochastic overbought
                if self.stoch.lines.percK[0] < self.stoch.lines.percK[-1]:  # Starting to turn down
                    self.log(f'STOCHASTIC OVERBOUGHT SIGNAL: K: {self.stoch.lines.percK[0]:.2f}, D: {self.stoch.lines.percD[0]:.2f}')
                    return True
    ...
    def next(self):
        ...
        if len(self) % 5 == 0:
            rsi_msg = f'RSI: {self.rsi[0]:.2f}' if self.is_rsi_valid() else 'RSI: Initializing'
            self.log(
        ...
        if not self.position:
            ...
            if self.should_buy():
                ...
                if not self.can_trade_now():
                    self.log(f'TRADE THROTTLED: Need to wait {self.p.trade_throttle_days} days between trades', level='debug')
                    return
                    
                size = self.calculate_position_size()
                if size > 0:
                    self.log(f'BUY CREATE: Price: {self.dataclose[0]:.2f}, Size: {size}, RSI: {self.rsi[0]:.2f}')
    ...
    def stop(self):
        ...
        self.log('      It performs poorly in strong trending markets where RSI remains extreme')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='RSI Overbought/Oversold Reversal Strategy for Sideways/Ranging Markets',
    ...
    parser.add_argument('--fromdate', '-f', default='2024-01-01',
                       help='Starting date in YYYY-MM-DD format')
    parser.add_argument('--todate', '-t', default='2024-12-31',
                       help='Ending date in YYYY-MM-DD format')
    parser.add_argument('--cash', '-c', type=float, default=100000.0,
                       help='Starting cash')
    
    # RSI parameters
    parser.add_argument('--rsi-period', '-rp', type=int, default=14,
                       help='Period for RSI calculation')
    parser.add_argument('--oversold', '-os', type=float, default=30,
                       help='Oversold threshold for RSI')
    parser.add_argument('--overbought', '-ob', type=float, default=70,
                       help='Overbought threshold for RSI')
    parser.add_argument('--confirmation', '-cf', type=int, default=2,
                       help='Number of bars for confirmation')
    
    # Stochastic parameters
    parser.add_argument('--use-stoch', '-us', action='store_true',
                       help='Use Stochastic Oscillator for confirmation')
    parser.add_argument('--stoch-period', '-sp', type=int, default=14,
                       help='Period for Stochastic calculation')
    parser.add_argument('--stoch-smooth', '-ss', type=int, default=3,
                       help='Smoothing period for Stochastic')
    
    # Exit parameters
    parser.add_argument('--use-stop', '-st', action='store_true',
                       help='Use stop loss')
    parser.add_argument('--stop-pct', '-sp', type=float, default=2.0,
                       help='Stop loss percentage')
    parser.add_argument('--use-trail', '-ut', action='store_true',
                       help='Enable trailing stop loss')
    parser.add_argument('--trail-pct', '-tp', type=float, default=1.0,
                       help='Trailing stop percentage')
    parser.add_argument('--take-profit', '-tkp', type=float, default=4.0,
    ...
    # Trade throttling
    parser.add_argument('--trade-throttle-days', '-ttd', type=int, default=1,
...
def main():
    ...
    print(f'RSI Parameters: Period={args.rsi_period}, Oversold={args.oversold}, Overbought={args.overbought}')
    print(f'Confirmation Bars: {args.confirmation}')
    
    if args.use_stoch:
        print(f'Using Stochastic ({args.stoch_period}/{args.stoch_smooth}) for confirmation')
    ...
    print('It performs poorly in strongly trending markets where RSI can remain in extreme')
    print('territories for extended periods. Best used in range-bound markets with clear')
