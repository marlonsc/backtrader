...
class RSIDivergenceStrategy(bt.Strategy, TradeThrottling):
    ...
    params = (
        ...
        ('min_stop_distance', 0.5), # Minimum distance between entry and stop as percentage
        ...
        ('use_percent_trailing', False), # Whether to use percentage-based trailing stop
        ('trailing_percent', 2.0),    # Trailing stop percentage
        ('trade_direction', 'long_only'),  # Trade direction: 'long_only', 'short_only', or 'both'
    ...
    def __init__(self):
        ...
        self.current_consecutive_losses = 0  # Correctly initialized as instance variable
    ...
    def notify_trade(self, trade):
        ...
        if trade.pnl < 0:
            ...
            if self.current_consecutive_losses >= self.max_consecutive_losses:
                self.log(f"WARNING: {self.max_consecutive_losses} consecutive losses reached")
        else:
    ...
    def set_exit_orders(self, entry_price, is_buy=True):
        ...
        if is_buy:
            ...
            if self.p.use_percent_trailing:
                ...
                self.trailing_stop = self.sell(exectype=bt.Order.StopTrail, 
                                              trailpercent=self.p.trailing_percent/100,
                                              size=self.position_size)
                self.log(f'Setting percentage trailing stop at {self.p.trailing_percent}%')
            else:
                ...
                self.trailing_stop = self.sell(exectype=bt.Order.StopTrail, 
                                              ...
                                              size=self.position_size)
                self.log(f'Setting trailing stop with initial distance of {trail_amount:.2f}')
        else:
            ...
            if self.p.use_percent_trailing:
                # Use percentage-based trailing stop
                self.trailing_stop = self.buy(exectype=bt.Order.StopTrail,
                                             trailpercent=self.p.trailing_percent/100, 
                                             size=self.position_size)
                self.log(f'Setting percentage trailing stop at {self.p.trailing_percent}%')
            else:
    ...
    def calculate_position_size(self, entry_price, stop_price):
        ...
        if current_equity < self.initial_cash * 0.95:  # 5% drawdown
            ...
            self.log(f"Reducing risk due to drawdown. Current equity: {current_equity:.2f}", level='info')
        ...
        if risk_per_share < min_distance:
            self.log(f"Stop too close to entry. Using minimum distance of {min_distance:.2f}", level='warning')
        ...
        cash_available = self.broker.getcash() * 0.95  # Use only 95% of available cash for safety
        percent_cap = min(cash_available * max_pct / entry_price, absolute_max_shares)
        
        if size > percent_cap:
            self.log(f"Position size reduced from {size:.0f} to {percent_cap:.0f} to respect position limits", level='info')
            size = percent_cap
        ...
        if size < 1:
            self.log("Calculated position size too small. Skipping trade.", level='warning')
            return 0
    ...
    def find_divergences(self):
        ...
        try:
            ...
            if len(self.price_lows) >= 2:
                ...
                if (price_low1 < price_low2 and  # Price made lower low
                    rsi_low1 > rsi_low2 and      # RSI made higher low
                    oversold_condition and       # RSI is in oversold territory
                    significant_price_diff and   # Price difference is significant
                    significant_rsi_diff):       # RSI difference is significant
                    
                    self.divergence_bull = True
                    self.log(f"Strong bullish divergence detected: Price {price_low2:.2f}->{price_low1:.2f}, RSI {rsi_low2:.2f}->{rsi_low1:.2f}", level='info')
            ...
            if len(self.price_highs) >= 2:
                ...
                if (price_high1 > price_high2 and  # Price made higher high
                    rsi_high1 < rsi_high2 and      # RSI made lower high
                    overbought_condition and       # RSI is in overbought territory
                    significant_price_diff and     # Price difference is significant
                    significant_rsi_diff):         # RSI difference is significant
                    
                    self.divergence_bear = True
                    self.log(f"Strong bearish divergence detected: Price {price_high2:.2f}->{price_high1:.2f}, RSI {rsi_high2:.2f}->{rsi_high1:.2f}", level='info')
    ...
    def next(self):
        ...
        if drawdown_pct > self.max_dd_percent:
            ...
            if not self.trading_suspended:
                self.log(f"TRADING SUSPENDED: Maximum drawdown of {drawdown_pct:.2f}% exceeded", level='warning')
        ...
        if self.position:
            ...
            if self.position.size > 0 and (current_rsi > self.p.exit_rsi_thresh or self.data.close[0] < self.data.low[1]):
                self.cancel_exit_orders()
                self.close()
            ...
            if not self.p.use_percent_trailing and self.trailing_stop is not None:
                try:
                    if self.position.size > 0:  # Long position
                        ...
                        stop_price = min(self.data.low.get(size=self.p.trailing_stop_lookback))
                        ...
                        stop_price = max(self.data.high.get(size=self.p.trailing_stop_lookback))
        ...
        if self.p.min_trend_strength > 0:
            ...
            if self.p.trade_direction in ['long_only', 'both'] and count_above_sma < self.p.min_trend_strength:
                trend_strength_met = False
            elif self.p.trade_direction in ['short_only', 'both'] and count_below_sma < self.p.min_trend_strength:
                trend_strength_met = False
        ...
        if (self.divergence_bull and
            self.p.trade_direction in ['long_only', 'both'] and
            trend_strength_met and 
            volume_confirmed and
            current_rsi >= self.p.min_rsi_value):  # Apply minimum RSI threshold
            ...
            rsi_moving_favorably = current_rsi > rsi_low1 + 3.0  # RSI moving up after bullish divergence
            price_confirming = self.data.close[0] > self.data.close[1]  # Price moving up
            
            # Only enter if we have confirmation
            if trend_up and rsi_moving_favorably and price_confirming and volume_confirmed:
                ...
                if size > 0:
                    # Enter long position
                    self.log(f"BULLISH DIVERGENCE SIGNAL: RSI={current_rsi:.2f}, Size={size}")
        ...
        if (self.divergence_bear and
            self.p.trade_direction in ['short_only', 'both'] and
            trend_strength_met and
            volume_confirmed and
            current_rsi <= self.p.max_rsi_value):  # Apply maximum RSI threshold
            ...
            if trend_down or sma_cross_down:
                ...
                if size > 0:
                    ...
                    self.log(f"BEARISH DIVERGENCE SIGNAL: RSI={current_rsi:.2f}, Size={size}")
...
def parse_args():
    ...
    parser.add_argument('--min-rsi-value', '-mrv',
                        default=20, type=int,
                        help='Minimum RSI value for bullish divergence')
    
    parser.add_argument('--max-rsi-value', '-maxrv',
                        default=80, type=int,
    ...
    parser.add_argument('--min-trend-strength', '-mts',
                        default=0, type=int,
                        help='Minimum consecutive bars in trend direction')
    
    parser.add_argument('--use-volume', '-uv',
                        action='store_true',
    ...
    parser.add_argument('--max-position-size', '-mps',
                        default=5.0, type=float,
                        help='Maximum position size as percentage of portfolio')
    
    parser.add_argument('--min-stop-distance', '-msd',
    ...
    parser.add_argument('--trailing-stop-lookback', '-tsl',
                        default=5, type=int,
                        help='Lookback period for trailing stop (bars)')
    
    parser.add_argument('--use-percent-trailing', '-upt',
                        action='store_true',
                        help='Use percentage-based trailing stop')
    
    parser.add_argument('--trailing-percent', '-tp',
                        default=2.0, type=float,
