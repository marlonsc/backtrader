...
def upsert_stock_detail(
    ...
    if os.path.exists(path):
        ...
        if int(start_date) > int(end_date):
            ...
            return
        stock_data = eval("ak.stock_" + type + "_hist")(
            symbol=symbol,
            period="daily",
        ...
        if stock_data.shape[0] < 1:  # return 200 but without data
            ...
            stock_data.index = range(
                old_df.shape[0], stock_data.shape[0] + old_df.shape[0]
            )
        ...
        if stock_data.shape[0] < 1:  # return 200 but without data
            print(
                f"{symbol}, download success, but without data, maybe the stock exit."
            )
        else:
            stock_data.columns = [
                "datetime",
...
if __name__ == "__main__":
    ...
    for stock_list in stock_lists:
        ...
        pool.apply_async(
            func=get_stock_list_task,
            args=(stock_list, type, start_date),
            callback=bar_update,
        )
