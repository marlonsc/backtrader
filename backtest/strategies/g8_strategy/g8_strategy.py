...
class MAStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ma1 = bt.indicators.SimpleMovingAverage(
            self.data, period=self.params.ma_period1
        )
        self.ma2 = bt.indicators.SimpleMovingAverage(
            self.data, period=self.params.ma_period2
        )
        self.highest = bt.indicators.Highest(
            self.data, period=self.params.price_period, subplot=False
        )
    ...
    def next(self):
        ...
        if not self.position:
            if (
                ...
                and self.is_cross_up()
                and self.data.close[0] >= self.highest[0]
                and self.get_percentage(self.data.close[0], self.data.open[0])
                > self.stat["middle"]
            ):
                # buy 1
            ...
            # rise too fast
            if self.data.close[0] >= self.ma1[0] * (1 + self.stat["high"] * 2 / 100):
                if self.data.close[0] < self.data.open[0] and (
                    self.get_percentage(self.data.close[-1], self.data.open[-1])
                    > self.stat["high"]
                    or (
                        self.get_percentage(self.data.close[-1], self.data.open[-1])
                        > self.stat["middle"]
                        and self.get_percentage(self.data.close[-2], self.data.open[-2])
                        > self.stat["middle"]
                    )
                ):
                    # sell 1
    ...
    def check_low_price(self):
        ...
        i = 0
        while (
            self.data.datetime.date(0 - i) > self.startDate
            and self.data.close[0 - i] < close * self.params.price_times
        ):
            i = i + 1
...
if __name__ == "__main__":
    ...
    for file in files:
        stock_id = file.split(".")[1]
        if (
            stock_id.startswith("3")
            or stock_id.startswith("4")
            or stock_id.startswith("8")
        ):
            continue
    ...
    print("---------------------")
    print(
        f"{stock_count} stocks, {trade_count} trades. {earn_trade_count} earns,"
        f" {loss_trade_count} losses"
    )
