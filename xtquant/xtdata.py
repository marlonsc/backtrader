...
def connect(ip="", port=None, remember_if_success=True):
    ...
    if __data_dir_from_server == "":
        __data_dir_from_server = _OS_.path.join(
            __client.get_app_dir(), default_data_dir
        )
...
def get_field_list(metaid):
    ...
    if not __meta_field_list:
        x = open(
            _OS_.path.join(_OS_.path.dirname(__file__), "config", "metaInfo.json"),
            "r",
            encoding="utf-8",
        ).read()
        metainfo = eval(x)
...
def get_stock_list_in_sector(sector_name, real_timetag=-1):
    ...
    if isinstance(real_timetag, str):
        if real_timetag != "":
            real_timetag = int(
                _TIME_.mktime(_TIME_.strptime(real_timetag, "%Y%m%d")) * 1000
            )
        else:
            real_timetag = -1
...
def get_financial_data(
    ...
    for sl in stock_list2:
        data2 = client.get_financial_data(
            sl, req_list, start_time, end_time, report_type
        )
        for s in data2:
            data[s] = data2[s]
...
def get_market_data_ori(
    ...
    return client.get_market_data3(
        field_list,
        stock_list,
        period,
        start_time,
...
def get_market_data(
    ...
    if period in {
        ...
        index, data = get_market_data_ori(
            field_list,
            stock_list,
            period,
            start_time,
...
def get_market_data_ex(
    ...
    if period == "hkbrokerqueue" or period == "hkbrokerqueue2" or period == (1820, 0):
        ...
        return get_broker_queue_data(
            stock_list, start_time, end_time, count, showbrokename
        )

    period = _get_tuple_period(period) or period
    if isinstance(period, tuple):
        return _get_market_data_ex_tuple_period(
            field_list,
            stock_list,
            period,
            start_time,
    ...
    if period in {
        ...
        return _get_market_data_ex_ori_221207(
            field_list,
            stock_list,
            period,
            start_time,
    ...
    query_field_list = (
        field_list
        if (not field_list) or (ifield in field_list)
        else [ifield] + field_list
    )
    ori_data = get_market_data_ex_ori(
        query_field_list,
        stock_list,
        period,
        start_time,
...
def _get_market_data_ex_221207(
    ...
    if period in {
        ...
        ori_data = _get_market_data_ex_ori_221207(
            query_field_list,
            stock_list,
            period,
            start_time,
...
def _get_data_file_path(stocklist, period, date="20380119"):
    ...
    path_result = _BSON_call_common(client.commonControl, "getdatafilepath", data)
...
def _needconvert_period(period):
    datas = {"snapshotindex": 3004, "etfiopv": 4011}
...
def _get_market_data_ex_tuple_period_ori(
    stock_list=[], period=(), start_time="", end_time="", count=-1
):
    client = get_client()
...
def _convert_component_info(data, convert_field_list):
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            new_data[name] = [
                _convert_component_info(item, convert_field_list) for item in value
            ]
        else:
            new_data[name] = value
...
def _get_market_data_ex_tuple_period(
    ...
    all_data = _get_market_data_ex_tuple_period_ori(
...
def get_l2_transaction(
    field_list=[], stock_code="", start_time="", end_time="", count=-1
):
    """
    level2逐笔成交
    """
...
def getDividFactors(stock_code, date):
    ...
    res = {
        resData[i]: [resData[i + 1][j] for j in range(0, len(resData[i + 1]), 1)]
        for i in range(0, len(resData), 2)
    }
...
def get_sec_main_contract(code_market: str, start_time: str = "", end_time: str = ""):
    ...
    if start_time == "" and end_time == "":
        ...
        if code.endswith("00"):
            return (
                client.get_main_contract(code + "1" + "." + marker_code)
                + "."
                + marker_code
            )
    elif start_time and end_time == "":
...
def subscribe_quote2(
    ...
    client = get_client()
    return client.subscribe_quote(
        _BSON_.BSON.encode(meta), _BSON_.BSON.encode(region), callback
    )
...
def subscribe_l2thousand(stock_code, gear_num=0, callback=None):
    ...
    region = {
        "thousandGearNum": gear_num,
        "thousandDetailGear": 0,
        "thousandDetailNum": 0,
    }
    ...
    if price is not None:
        if isinstance(price, float):
            ...
            price = [
                i
                for i in range(
                    int(sprice * 10000), int((eprice + 0.01) * 10000), int(0.01 * 10000)
                )
            ]
        else:
...
def remove_stock_from_sector(sector_name, stock_list):
    ...
    data["sectorname"] = sector_name
    data["stocklist"] = stock_list
    result_bson = client.commonControl(
        "removestockfromsector", _BSON_.BSON.encode(data)
    )
    result = _BSON_.BSON.decode(result_bson)
    return result.get("result")
...
def _download_history_data_by_metaid(
    stock_code, metaid, period, start_time="", end_time="", incrementally=True
):
    cl = get_client()
...
def download_history_data(
    ...
    if __download_version is None:
        ...
        if isinstance(period, tuple):
            metaid, periodNum = period
            return _download_history_data_by_metaid(
                stock_code, metaid, periodNum, start_time, end_time, incrementally
            )

        return _download_history_data(stock_code, period, start_time, end_time)
    else:
        return download_history_data2(
            [stock_code], period, start_time, end_time, None, incrementally
        )
...
def download_history_data2(
    ...
    result = client.supply_history_data2(
        stock_list, period, start_time, end_time, _BSON_.BSON.encode(param), on_progress
    )
    if not result:
        import time
...
def download_financial_data(
    ...
    for table in table_list:
        download_history_data2(
            stock_list, table, start_time, end_time, None, incrementally
        )
...
def get_instrument_type(stock_code, variety_list=None):
    ...
    v_dct1 = {}
    if (
        variety_list == None or len(variety_list) == 0
    ):  # 返回该stock_code所有的品种的T/None(False)
...
def download_holiday_data(incrementally=True):
    ...
    inst = _BSON_call_common(
        cl.commonControl, "downloadholidaydata", {"incrementally": incrementally}
    )
    return inst
...
def get_trading_time(stockcode):
    ...
    inst = _BSON_call_common(
        cl.commonControl, "gettradingtime", {"market": market, "code": code}
    )
...
def get_option_detail_data(optioncode):
    ...
    if (
        ...
        or market == "SZO"
        or (
            (market == "CFFEX" or market == "IF")
            and inst.get("InstrumentID").find("-") >= 0
        )
        or (
            market in ["SF", "SHFE", "DF", "DCE", "INE", "GF", "GFEX", "ZF", "CZCE"]
            and inst.get("ExtendInfo", {}).get("OptionType") in [0, 1]
        )
    ):
        field_list = [
            "ExchangeID",
...
def get_option_undl_data(undl_code_ref):
    ...
    if undl_code_ref:
        ...
        if undl_code_ref.endswith(".SH"):
            if (
                undl_code_ref == "000016.SH"
                or undl_code_ref == "000300.SH"
                or undl_code_ref == "000852.SH"
                or undl_code_ref == "000905.SH"
            ):
...
def get_his_option_list_batch(undl_code, start_time="", end_time=""):
    ...
    for timetag in date_list:
        ...
        data2 = (
            data_all.loc[data_all.eval(select)]
            .reset_index()
            .drop(["index", "time"], axis=1)
        )
        if data2.empty:
...
def get_his_st_data(stock_code):
    fileName = _OS_.path.join(
        get_data_dir(), "..", "data", "SH_XXXXXX_2011_86400000.csv"
    )
...
def subscribe_formula(
    ...
    result = _BSON_.BSON.decode(
        cl.commonControl("createrequestid", _BSON_.BSON.encode({}))
    )
...
def reset_market_trading_day_list(market, datas):
    ...
    result = _BSON_call_common(
        cl.custom_data_control, "createmarketchange", {"market": market}
    )
    cid = result["cid"]

    result = _BSON_call_common(
        cl.custom_data_control,
        "addtradingdaytochange",
        {"cid": cid, "datas": datas, "coverall": True},
    )
...
def push_custom_data(meta, datas, coverall=False):
    ...
    result = _BSON_call_common(
        cl.custom_data_control,
        "pushcustomdata",
        {"meta": meta, "datas": datas, "coverall": coverall},
    )
    return
...
def get_period_list():
    client = get_client()
    result = _BSON_.BSON.decode(
        client.commonControl("getperiodlist", _BSON_.BSON.encode({}))
    )
...
def gen_factor_index(
    ...
    生成因子指数扩展数据
    """
    running_info = {"result": {}, "finished": 0, "total": 0}
...
def create_formula(formula_name, formula_content, formula_params={}):
    ...
    return _BSON_call_common(get_client().commonControl, "createformula", data)
    ...
    if sys.version_info.major > 2:
        ...
        if sys.version_info.minor > 6:
            table = feather.read_table(
                source=file_path, columns=None, memory_map=True, use_threads=True
            )
        else:
...
def write_feather(dest_path, param, df):
    ...
    if sys.version_info.major > 2:
        ...
        schema = Schema.from_pandas(df).with_metadata(
            {"param_bin": _BSON_.BSON.encode(param), "param_str": json.dumps(param)}
        )
...
class QuoteServer:
    ...
    def test_load(self):
        ...
        result = self._BSON_call_common(
            cl.commonControl,
            "testload",
            {"ip": self.info["ip"], "port": self.info["port"], "info": self.info},
        )
        return result
...
def fetch_quote(root_path, key_list):
    ...
    cl = get_client()
    inst = _BSON_call_common(cl.commonControl, "fetchquote", {"dir": root_path})

    config_dir = _OS_.path.join(
        root_path, "userdata_mini", "users", "xtquoterconfig.xml"
    )
    ...
    for key_servers in servers.items():
        for qs in key_servers[1]:
            if (
                qs.info["load"] != 1000000.0
                and qs.info["delay"] != 1000000.0
                and qs.info["accessible"]
            ):
        ...
        for data in data_list:
            ...
            if "成份股信息" in convert_data:
                for sub_data in convert_data["成份股信息"]:
                    if (
                        "成份股所属市场" in sub_data
                        and "成份股代码" in sub_data
                        and str(sub_data["成份股所属市场"]) in convert_market
                    ):
                        sub_data["成份股代码"] = (
                            sub_data["成份股代码"]
                            + "."
                            + convert_market[str(sub_data["成份股所属市场"])]
                        )
                        sub_data["成份股所属市场"] = convert_market[
...
def download_his_st_data():
    ...
    result = _BSON_call_common(cl.commonControl, "downloadhisstdata", {})
    return result
...
def get_hk_broker_dict():
    ...
    if not __hk_broke_info:
        data = _get_market_data_ex_tuple_period_ori(
            ["XXXXXX.HK"], (2038, 86401000), "", ""
        )
...
def get_broker_queue_data(
    stock_list=[], start_time="", end_time="", count=-1, show_broker_name=False
):
    ori_data = get_market_data_ex_ori(
        [], stock_list, "hkbrokerqueue", start_time, end_time, count
    )
        ...
        if not status.get("done", True):
            ...
            while not status.get("done", True):
                ...
                status = _BSON_call_common(
                    cl.commonControl, "querytaskstatus", {"taskid": taskid}
                )
...
def download_tabular_data(
    ...
    result = _BSON_call_common(cl.commonControl, "downloadtabulardata", param)
...
def get_trading_period(stock_code):
    ...
    result = _BSON_call_common(
        cl.commonControl, "getopenclosetradetimebystock", {"stockMarket": stock_code}
    )
...
def get_kline_trading_period(stock_code):
    ...
    result = _BSON_call_common(
        cl.commonControl, "getopencloseklinetimebystock", {"stockMarket": stock_code}
    )
...
def get_all_trading_periods():
    ...
    result = _BSON_call_common(cl.commonControl, "getopenclosealltradetime", {})
...
def get_all_kline_trading_periods():
    ...
    result = _BSON_call_common(cl.commonControl, "getopencloseallklinetime", {})
...
def get_authorized_market_list():
    ...
    """
    return _BSON_call_common(
        get_client().commonControl, "getauthorizedmarketlist", {}
    ).get("result", [])
