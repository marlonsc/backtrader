...
if __rpc_init_status < 0:
    raise Exception(
        f"rpc init failed, error_code:{__rpc_init_status}, configdir:{__rpc_config_dir}"
    )
...
Args:
    start_local_service: (Default value = True)"""
    import time
    ...
    for mkt, boinfo in result.items():
        ...
        if info["done"] == 1:
            if info["errorcode"] != 0:
                srv_addr = (
                    info["loginparam"]["ip"] + ":" + str(info["loginparam"]["port"])
                )
                error = info["boerror"]
            ...
            if info["resultcode"] != 0:
                ...
                raise ConnectionError(
                    f"行情连接初始化异常 {mkt} {srv_addr} {error} {reason}"
                )
        else:
            status = bson.decode(__dc.fetch_server_list_status())
