...
class StrategyLoader:
    ...
    def init(this):
        ...
        if not C.title:
            C.title = os.path.basename(
                os.path.abspath(C.user_script).replace(".py", "")
            )
        ...
        if C.start_time:
            C.start_time_str = (
                C.start_time.replace("-", "").replace(" ", "").replace(":", "")
            )
            C.start_time_num = int(datetime_to_timetag(C.start_time_str))
        if C.end_time:
            C.end_time_str = (
                C.end_time.replace("-", "").replace(" ", "").replace(":", "")
            )
        ...
        if 1:  # post initcomplete
            ...
            if C.start_time:
                ...
                C.start_time_num = int(datetime_to_timetag(C.start_time_str))
                init_result["backtest"]["start_time"] = dt.datetime.fromtimestamp(
                    C.start_time_num / 1000
            ...
            if C.end_time:
                ...
                C.end_time_num = int(datetime_to_timetag(C.end_time_str))
                init_result["backtest"]["end_time"] = dt.datetime.fromtimestamp(
                    C.end_time_num / 1000
                ).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def run_bar(this):
        ...
        for i in range(max(C.lastrunbarpos, 0), len(C.timelist)):
            ...
            if (not C.start_time_num or C.start_time_num <= bartime) and (
