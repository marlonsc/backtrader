...
def get_market_data(
    ...
    if period == "tick":
        ...
        for stock in data:
            ...
            pd_data["stime"] = [
                timetag_to_datetime(t, stime_fmt) for t in pd_data["time"]
            ]
    ...
    if (
        len(fields) == 1
        and len(stock_code) <= 1
        and ((start_time == "" and end_time == "") or start_time == end_time)
        and (count == -1 or count == -2)
    ):
        # if resultDict:
        ...
        for field in field_list:
            ...
            for row_data in table_data:
                if row_data.get(report_type, None) == None:
                    continue
                date = time.strftime(
                    "%Y%m%d", time.localtime(row_data[report_type] / 1000)
                )
...
def get_trade_detail_data(accountid, accounttype, datatype, strategyname=""):
    ...
    client = xtdata.get_client()
    result_bson = client.callFormula(
        request_id, "gettradedetail", _BSON_.BSON.encode(data)
    )
...
def _get_callback_cache_impl(type, requestid):
    ...
    client = xtdata.get_client()
    result_bson = client.callFormula(
        requestid, "getcallbackcache", _BSON_.BSON.encode(data)
    )
...
def calc_bsm_price(
    optionType, strikePrice, targetPrice, riskFree, sigma, days, dividend, requestid
):
    data = {}
    ...
    client = xtdata.get_client()
    result_bson = client.callFormula(
        requestid, "calcbsmprice", _BSON_.BSON.encode(data)
    )
    result = _BSON_.BSON.decode(result_bson)
...
def call_formula_batch(
    ...
    result = client.commonControl(
        "callformulabatch", _BSON_.BSON.encode({"params": params})
    )
...
def is_suspended_stock(stock_code, period, timetag):
    ...
    result = client.commonControl(
        "issuspendedstock",
        _BSON_.BSON.encode(
            {"stockcode": stock_code, "period": period, "timetag": timetag}
        ),
    )
    result = _BSON_.BSON.decode(result)
