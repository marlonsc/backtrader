...
class DynamicSpreadRSI_MACD_Strategy(bt.Strategy):
    ...
    def next(self):
        ...
        if (
            ...
            or len(self.macd) < self.p.macd_slow + 2
        ):
            return
...
def run_strategy(
    ...
    # 添加策略
    cerebro.addstrategy(
        DynamicSpreadRSI_MACD_Strategy,
...
def grid_search():
    ...
    if results:
        ...
        print(
            ...
            f" 慢线={best_result['params']['macd_slow']},"
            f" 信号线={best_result['params']['macd_signal']}"
        )
        ...
        for i, result in enumerate(sorted_results[:10]):  # 只显示前10个最好的结果
            print(
                ...
                f" sharpe={result['sharpe']:.4f}, drawdown={result['drawdown']:.2f}%,"
                f" return={result['returns']:.2f}%, win_rate={result['win_rate']:.2f}%"
            )
    else:
        print("未找到有效的参数组合")
