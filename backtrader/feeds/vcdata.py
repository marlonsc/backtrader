...
class VCData(with_metaclass(MetaVCData, DataBase)):
    ...
    def setenvironment(self, env):
        """Receives an environment (cerebro) and passes it over to the store it belongs
        to."""
        super().setenvironment(env)
        env.addstore(self.store)

    def start(self):
        """Starts the VC connecction and gets the real contract and contractdetails if it
        exists."""
        super().start()
    ...
    def _getpingtmout(self):
        """Returns the actual ping timeout for PumpEvents to wake up and call ping, which
        will check if the not yet delivered bar can be delivered.

        The bar may be stalled because vc awaits a new tick and during low negotiation
        hour this can take several seconds after the actual expected delivery time
        """
        if self._ticking:
            return -1  # no timeout
