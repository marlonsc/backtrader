...
class Strategy(with_metaclass(MetaStrategy, StrategyBase)):
    ...
    def _addwriter(self, writer):
        """Unlike the other _addxxx functions this one receives an instance because the
        writer works at cerebro level and is only passed to the strategy to simplify the
        logic."""
    ...
    def _addanalyzer_slave(self, ancls, *anargs, **ankwargs):
        """Like _addanalyzer but meant for observers (or other entities) which rely on the
        output of an analyzer for the data. These analyzers have not been added by the
        user and are kept separate from the main analyzers.
    ...
    def _clk_update(self):
        ...
        if any(new_len > old_len for old_len, new_len in zip(self._dlens, newdlens)):
            self.forward()
    ...
    def getwritervalues(self):
        ...
        for iocsv in self.indobscsv:
            ...
            values.append(lio)
            if lio:
                values.extend(map(lambda item: item[0], iocsv.lines.itersize()))
            else:
