...
class TradingCalendarBase(with_metaclass(MetaParams, object)):
    ...
    def schedule(self, day):
        """Returns a tuple with the opening and closing times (``datetime.time``) for the
        given ``date`` (``datetime/date`` instance)"""
        raise NotImplementedError
    ...
    def last_weekday(self, day):
        """Returns ``True`` if the given ``day`` (datetime/date) instance is the last
...
class TradingCalendar(TradingCalendarBase):
    ...
    def schedule(self, day, tz=None):
        """Returns the opening and closing times for the given ``day``. If the method is
        called, the assumption is that ``day`` is an actual trading day.
