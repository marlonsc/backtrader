...
class LineBuffer(LineSingle):
    """LineBuffer defines an interface to an "array.array" (or list) in which index 0
    points to the item which is active for input and output.
    ...
    def minbuffer(self, size):
        ...
        In non-dqbuffer mode, this is always true (of course until data is filled at the
        beginning, there are less values, but minperiod in the framework should account
        for this.

        In dqbuffer mode the buffer has to be adjusted for this if currently less than
        requested
        """
    ...
    def buflen(self):
        ...
        The internal buffer can be longer than the actual stored data to allow for
        "lookahead" operations. The real amount of data that is held/can be held in the
        buffer is returned
        """
    ...
    def home(self):
        ...
        The underlying buffer remains untouched and the actual len can be found out with
        buflen
        """
        self.idx = -1
        self.lencount = 0
    ...
    def bind2lines(self, binding=0):
        """Stores a binding to another line.

        "binding" can be an index or a name
        """
    ...
    def __call__(self, ago=None):
        """Returns either a delayed verison of itself in the form of a LineDelay object or
        a timeframe adapting version with regards to a ago.
...
class MetaLineActions(LineBuffer.__class__):
    ...
    Scans the instance before init for LineBuffer (or parentclass LineSingle) instances to
    calculate the minperiod for this instance

    postinit it registers the instance to the owner (remember that owner has been found in
    the base Metaclass for LineRoot)
    """
...
class LineActions(with_metaclass(MetaLineActions, LineBuffer)):
    """Base class derived from LineBuffer intented to defined the minimum interface to
    make it compatible with a LineIterator by providing operational _next and _once
    interfaces.
...
class _LineForward(LineActions):
    """Takes a LineBuffer (or derived) object and stores the value from "ago" periods from
    the future."""
...
class LinesOperation(LineActions):
    ...
    To optimize the operations and avoid conditional checks the right next/once is chosen
    using the operation direction (normal or reversed) and the nature of the operands
    (LineBuffer vs non-LineBuffer)

    In the "once" operations "map" could be used as in:

    operated = map(self.operation, srca[start:end], srcb[start:end]) self.array[start:end]
    = array.array(str(self.typecode), operated)

    No real execution time benefits were appreciated and therefore the loops have been
    kept in place for clarity (although the maps are not really unclear here)
    """
...
class LineOwnOperation(LineActions):
    ...
    It will "next"/traverse the array applying the operation and storing the result in
    self
    """
