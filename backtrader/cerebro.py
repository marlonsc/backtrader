...
class Cerebro(with_metaclass(MetaParams, object)):
    ...
    def set_fund_history(self, fund):
        """Add a history of orders to be directly executed in the broker for performance
        evaluation.
    ...
    def notify_timer(self, timer, when, *args, **kwargs):
        ...
        by ``add_timer``, and ``when`` is the calling time. ``args`` and ``kwargs`` are
        any additional arguments passed to ``add_timer``
    ...
    def _add_timer(
        ...
        **kwargs,
        ...
        """Internal method to really create the timer (not started yet) which can be
        called by cerebro instances or other objects which can access cerebro."""
    ...
    def addcalendar(self, cal):
        """Adds a global trading calendar to the system. Individual data feeds may have
    ...
    def addwriter(self, wrtcls, *args, **kwargs):
        """Adds an ``Writer`` class to the mix.

        Instantiation will be done at
        ``run`` time in cerebro
        """
    ...
    def addsizer_byidx(self, idx, sizercls, *args, **kwargs):
        """Adds a ``Sizer`` class by idx.

        This idx is a reference compatible to
    ...
    def addindicator(self, indcls, *args, **kwargs):
        """Adds an ``Indicator`` class to the mix.
    ...
    def addanalyzer(self, ancls, *args, **kwargs):
        """Adds an ``Analyzer`` class to the mix.
    ...
    def addobserver(self, obscls, *args, **kwargs):
        """Adds an ``Observer`` class to the mix.
    ...
    def addobservermulti(self, obscls, *args, **kwargs):
        """Adds an ``Observer`` class to the mix. Instantiation will be done at ``run``
        time.

        It will be added once per "data" in the system. A use case is a buy/sell observer
    ...
    def __call__(self, iterstrat):
        """Used during optimization to pass the cerebro over the multiprocesing module
        without complains."""
    ...
    def __getstate__(self):
        """Used during optimization to prevent optimization result `runstrats` from being
        pickled to subprocesses."""
    ...
    def runstop(self):
        """If invoked from inside a strategy or anywhere else, including other threads the
        execution will stop as soon as possible."""
    ...
    def _brokernotify(self):
        """Internal method which kicks the broker and delivers any broker notification to
        the strategy."""
        self._broker.next()
        while True:
