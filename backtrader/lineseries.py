...
class LineAlias:
    ...
    def __set__(self, obj, value):
        """A line cannot be "set" once it has been created.

        But the values inside the line can be "set". This is achieved by adding a binding
...
class Lines:
    """Defines an "array" of lines which also has most of the interface of a LineBuffer
    class (forward, rewind, advance...).

    This interface operations are passed to the lines held by self

    The class can autosubclass itself (_derive) to hold new lines keeping them in the
    defined order.
    """
    ...
    def _derive(
        ...
        """Creates a subclass of this class with the lines of this class as initial input
        for the subclass. It will include num "extralines" and lines present in
        "otherbases".

        "name" will be used as the suffix of the final class name

        "linesoverride": if True the lines of all bases will be discarded and the
        baseclass will be the topmost class "Lines". This is intended to create a new
        hierarchy
        """
        obaseslines = ()
        obasesextralines = 0
...
class MetaLineSeries(LineMultiple.__class__):
    ...
    def donew(cls, *args, **kwargs):
        """Intercept instance creation, take over lines/plotinfo/plotlines class
        attributes by creating corresponding instance variables and add aliases for
        "lines" and the "lines" held within it."""
        ...
        for line_idx, line in enumerate(_obj.lines):
            setattr(_obj, "line_%s" % line_idx, _obj._getlinealias(line_idx))
            setattr(_obj, "line_%d" % line_idx, line)
            setattr(_obj, "line%d" % line_idx, line)
